...
public:
	olsEngine3D()
	{
		m_sAppName = L"First 3D Graphics";
	}
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (GetKey(L'W').bHeld || GetKey(VK_UP).bHeld)
			vCamera = Vector_Add(vCamera, vForward);

		if (GetKey(L'S').bHeld || GetKey(VK_DOWN).bHeld)
		...
		//fTheta += 1.0f * fElapsedTime; // Rotate the object
	...
	void TexturedTriangle(int x1, int y1, float u1, float v1, float w1, int x2, int y2, float u2, float v2, float w2, int x3, int y3, float u3, float v3, float w3, olcSprite* tex)
	{
		if (y2 < y1)
		{
			swap(y1, y2);
...
{
	olsEngine3D demo;
	if (demo.ConstructConsole(300, 240, 4, 4))
		demo.Start();
